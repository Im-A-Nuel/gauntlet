...
## CLI hardening checkpoint: both remaining findings from docs/COORDINATOR_NOTES.md closed

Scope for this checkpoint was exactly the two items Codex's review left open — nothing else touched (no dashboard, no root shared docs beyond this file and COORDINATOR_NOTES.md, no Git operations).

### 1. `strengthen` lock leak on error (`cmd/gauntlet/strengthen.go`, `cmd/gauntlet/root.go`)

The bug: `strengthen`'s `RunE` called `bob.Acquire` then `defer release()`, but every failure after that point (snapshot error, Bob invocation failure, verification violation, re-run failure, scope-drift abort — six call sites) called `die()`, which calls `os.Exit` directly. Go runs no deferred functions on `os.Exit`, so any of those six failures left `.gauntlet/.lock` on disk permanently, bricking every subsequent `run`/`strengthen` until a human deleted the file by hand. Since Bob isn't installed in this environment, the very first of those six paths (Bob invocation failing because the executable doesn't exist) is close to guaranteed on a real attempt — this wasn't an edge case.

Fix:
- `cmd/gauntlet/root.go` gained an `*exitError` type (an error carrying a process exit code) and an `exitErrorf` constructor, plus `main` now unwraps one via `errors.As` before calling `die` — so a command can return a normal Go `error` and still get a specific exit code (docs/SCHEMA.md §7), rather than being forced to call `die`/`os.Exit` directly to express anything other than exit code 3. `die` itself is untouched and still used by every command's *pre-lock* setup (`repoRoot`, config load, etc.) exactly as before — those points have no pending defer, so calling it there was never the bug.
- `cmd/gauntlet/strengthen.go`'s `RunE` now does nothing but acquire the lock, `defer release()`, and `return runLockedStrengthen(...)` — a plain Go return. The extracted `runLockedStrengthen` function contains everything that used to call `die` after the lock, and now returns `exitErrorf(...)` on every one of those six paths instead. Because it's a normal return all the way up through `RunE`, the deferred `release()` always runs before the process can exit, on every path, success or failure.
- Regression test: `cmd/gauntlet/main_test.go`'s `TestStrengthenMissingBobExecutableLeavesNoLock` hand-writes a valid run artifact with one surviving mutant via the real `report` package (no Stryker run involved), points `--bob-cmd` at an executable that cannot exist, and asserts both the exit code (`ExitRunFailed`) and that `.gauntlet/.lock` is gone afterward.

### 2. `internal/report.Read` didn't validate artifacts before gate/report trusted them

The bug: `Read` was a bare `json.Unmarshal` — any syntactically valid JSON matching the `Artifact` shape was accepted, including a hand-forged file with a fabricated `trustScore`, mismatched counts, an unknown mutant status, or a `--run-id` value that path-traversed outside `.gauntlet/runs/`.

Fix — `internal/report/validate.go`, a new `(Artifact) Validate() error` method that `Read` now calls on every load (so `gate`, `report`, `strengthen`'s `report.Latest`, and `orchestrate.Run`'s own internal `report.Latest` call for `comparedTo` are all covered from one place), plus two checks inside `Read` itself:
- **Path safety**: a new `runIDPattern` (`^[A-Za-z0-9_-]+$`) is enforced in `pathFor` (used by both `Read` and `Write`) before any filesystem path is built from a run ID — closes the `--run-id ../../../etc/passwd` traversal.
- **Filename/content agreement**: `Read` rejects an artifact whose own `runId` field disagrees with the run ID it was requested/found under.
- **Structural validation**: `schemaVersion` must equal the current constant; `createdAt` must parse as RFC3339; `baseRef`/`headSha` non-empty; `trigger` must be one of the four documented values; every mutant `status` must be one of the seven documented values; mutant IDs unique within their file (docs/SCHEMA.md §2 only requires per-file uniqueness); `threshold`, `totals.trustScore`, `totals.lineCoverage` all finite and in `[0, 100]` (or null where the schema allows it); `totals.durationMs` non-negative; every path in `changedFiles` and `files[].path` is a safe repo-relative forward-slash path with no `..` segment and no drive letter; every `files[].path` must actually appear in `changedFiles`; `comparedTo`, if present, must itself be a path-safe run ID.
- **The core anti-forgery check**: every per-file `trustScore` and the entire `totals` block (mutants/killed/survived/timeout/noCoverage/ignored/compileError/runtimeError/trustScore) are recomputed from the artifact's own raw per-mutant statuses using `score.go`'s existing `BuildFileResult`/`BuildTotals` (the same code the CLI uses to write an artifact in the first place) and compared for exact equality against the stored values. A stored count or score that doesn't match what its own mutant list implies is rejected outright — this is what actually closes the "syntactically valid JSON with a fabricated score" hole.
- Table-driven negative tests in `internal/report/validate_test.go` (`TestValidateRejectsKnownDefects`, 24 cases) cover: unsupported schema version, unknown mutant status, invalid/malformed timestamp, empty baseRef/headSha, unknown trigger, threshold out of range and NaN, coverage out of range, negative duration, forged totals score, forged totals killed/survived counts, forged per-file score, `changedFiles` path traversal, a `files` entry not listed in `changedFiles`, duplicate mutant ID within a file, empty mutant ID, negative mutant line, a path-traversal run ID, and a path-traversal `comparedTo`. `TestReadRejectsOnDisk` covers the two `Read`-specific checks (traversal run ID, path-separator run ID, filename/content run-ID mismatch, plain not-found) against real files on disk.
- Two pre-existing tests in `internal/report/artifact_test.go` (`TestWriteReadRoundTrip`, `TestLatestPicksMostRecentByRunID`) constructed artifacts with only `RunID`/`SchemaVersion` set; those now fail the new validation on `Read`, so they were updated to build a real, self-consistent artifact via a shared `validArtifact` helper (same `BuildFileResult`/`BuildTotals` pattern used everywhere else). No test's *intent* changed, only its fixture.

Verified against real, already-on-disk artifacts (no new Stryker run for this checkpoint): `gauntlet --repo demo-repo report` and `gauntlet --repo demo-repo report --run-id 1789317267750-d1f85d4` both still read genuine prior runs correctly under the new strict validation; `gauntlet --repo demo-repo gate --min-score 80` correctly reports `stale_artifact` (repo HEAD has moved past the artifact's recorded HEAD from Codex's later dashboard commits — expected, not a validation regression); `gauntlet --repo demo-repo report --run-id "../../../../etc/passwd"` is rejected at exit code 3 with an explicit "invalid run ID" message.

Full verification for this checkpoint: `gofmt -l .` clean, `go build ./...` clean, `go vet ./...` clean, `go build -o ... ./cmd/gauntlet` (a real compiled binary, not just `go run`) succeeds, `go test ./...` passes across all 8 packages (`cmd/gauntlet`, `internal/{bob,config,gate,mutate,orchestrate,report,scope}`).
