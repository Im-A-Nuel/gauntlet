    ...
    files: [
      {
        path: "src/a.ts",
        trustScore: 66.7,
        mutants: ["killed", "timeout", "survived"].map((status, i) => ({
          id: String(i),
...
test("timeouts count as detection; an empty denominator is unscored", () => {
  assert.equal(score(24, 2, 34), 43.3);
  assert.equal(score(0, 0, 0), null);
  assert.equal(verdict(parseRun(fixture())), "fail");
});
test("inconsistent measurements and unknown statuses are rejected", () => {
  const bad = fixture();
  bad.totals.trustScore = 100;
  assert.throws(() => parseRun(bad));
  const badStatus = fixture();
  badStatus.files[0].mutants[0].status = "pending";
  assert.throws(() => parseRun(badStatus));
  const duplicate = fixture();
  duplicate.files[0].mutants[1].id = "0";
  assert.throws(() => parseRun(duplicate));
});
test("comparison distinguishes changed source scope and revisions", () => {
  const a = parseRun(fixture()),
    b = parseRun(fixture());
  b.headSha = "different";
  assert.equal(compare(a, b).comparable, false);
  assert.equal(compare(a, a).delta.trustScore, 0);
});
